...
class ReadFiles(object):
    ...
    def getEssentialGeneAnnoInfo(self):
        ...
        for index, row in AnnotationInfo.iterrows():
            self.essentialAnnotationInfoDict[row['#Gene_Name']] = [row['#COG'], row['#Organism'], row['#Class']]
            #print(self.essentialAnnotationInfoDict)
            #sys.exit()
        ...
        # for plot
        fopen_ess_gene_count = open('output/' + 'Accession_wise_ness_gene_count.txt', 'w')
    ...
    def getNonEssentialGeneAnnoInfo(self):
        ...
        for index, row in AnnotationInfo.iterrows():
            self.nonEssentialAnnotationInfoDict[row['#Gene_Name']] = [row['#COG'], row['#Organism'], row['#Class']]
        ...
        fopen_ness_gene_count = open('output/' + 'Accession_wise_ness_gene_count.txt', 'w')
