...
def combineAndSplitData(EssentialGeneFeatTable, NonEssentialGeneFeatTable, trainingProp, f_e, f_ne):
    ...
    essential_gene_length_list = np.array(EssentialGeneFeatTable)[:, ]
    non_essential_gene_length_list = np.array(NonEssentialGeneFeatTable)[:, ]
    ...
    for i in range(corr.shape[0]):
        for j in range(i, corr.shape[1]):
            if (corr[i][j] >= 0.95 or corr[i][j] <= -0.95) and i != j:
                print(corr[i][j])
...
def resizeData(table1, table2):
    ...
    ####################################
    #matrix1 = np.asarray(table1)
    #matrix2 = np.asarray(table2)
    #  np.atleast_2d(matrix1)
    #  np.atleast_2d(matrix2)
    #en dto my update
    ...
    numSampleToSelect = int((sampleSize * 95) / 100)
