...
class FeatureProcessingSeq(object):

    def __init__(self, read,geneSeq,protSeq):
        super(FeatureProcessingSeq, self).__init__()
        #initailisation  des seqGene et SeqProt
    ...
    def getFeaturesSeq(self, bins,seqName):
        ...
        if len(self.geneSeq) > self.maxEssentialGeneLength:
            self.maxEssentialGeneLength = len(self.geneSeq)

        # store gene sequence length
        self.EssentialGeneSeqLenghtFeatDict[seqName] = len(self.geneSeq)

        # get CAI and Max RCSU from the sequence
        CAI, RCSUMax = getCAIRCSU(self.geneSeq)
        self.EssentialCAIRCSUFeatDict[seqName] = [CAI, RCSUMax]

        # get protein seq feature
        proteinFeature = getProteinFeat(self.protSeq)
        ...
        binnedSeqFeat = list()
        binnedGCFeat = list()
        binnedSeqLength = len(self.geneSeq) / bins
        startPos = 0
        endPos = int(binnedSeqLength)
        for i in range(0, bins):
            if endPos > len(self.geneSeq):
                binnedSequence = self.geneSeq[startPos:]
            else:
                binnedSequence = self.geneSeq[startPos:endPos]
                startPos = endPos
...
def getCAIRCSU(sequence):
    ...
    for i in range(0, len(sequence), 3):
        codon = sequence[i: i + 3]
        if codon in codonIndexDict and codon not in ['ATG', 'TGG','TAG', 'TGA', 'TAA']:
